...
export async function getGrupoDashboard(grupoId: number): Promise<GrupoDashboardResponse> {
  const { data } = await httpClient.get<GrupoDashboardResponse>(`/dashboard/grupo/${grupoId}/`)
  return data
}
