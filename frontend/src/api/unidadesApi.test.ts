...
describe("unidadesApi", () => {
  it("sends supported unit filters to the API", async () => {
    ...
    expect(httpClient.get).toHaveBeenCalledWith("/unidades/", {
      params: { search: "Tropa", estado: "ACTIVA" },
    });
  });
  ...
  it("uses the exact structural option and command contracts", async () => {
    ...
    expect(httpClient.get).toHaveBeenCalledWith(
      "/unidades/opciones/beneficiarios/",
      { params: { subgrupo_id: 7, page: 2 } },
    );
    expect(httpClient.get).toHaveBeenCalledWith(
    ...
    expect(httpClient.patch).toHaveBeenCalledWith(
      "/unidades/subgrupos-miembros/9/reasignacion/",
      { subgrupo: 11 },
    );
    expect(httpClient.patch).toHaveBeenCalledWith(
      "/unidades/adultos-roles/4/",
      { rol: "ASISTENTE" },
    );
    expect(destinations.data[0]).toMatchObject({
      unidad: 2,
      unidad_nombre: "Tropa",
    });
  });
