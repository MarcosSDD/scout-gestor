...
export async function getGrupo(grupoId: number): Promise<GrupoDetailResponse> {
  const { data } = await httpClient.get<GrupoDetailResponse>(`/grupos/${grupoId}/`)
  return data
}

export async function getGrupoEstructura(grupoId: number): Promise<GrupoEstructuraResponse> {
  const { data } = await httpClient.get<GrupoEstructuraResponse>(`/grupos/${grupoId}/estructura/`)
  return data
}
