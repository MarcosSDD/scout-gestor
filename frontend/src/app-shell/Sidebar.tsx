...
export function Sidebar({ isOpen, onClose, onLogout }: SidebarProps) {
  ...
  const managementLinks = visibleItems.filter(
    (item) => item.group === "Gestion",
  );
  const operationLinks = visibleItems.filter(
    (item) => item.group === "Operativo",
  );
  ...
  return (
    <>
      <button
        className={`shell-backdrop ${isOpen ? "shell-backdrop--visible" : ""}`}
        type="button"
      ...
      <aside
        ref={sidebarRef}
        id="shell-sidebar"
        className={`shell-sidebar ${isOpen ? "nav-active" : ""}`}
        ...
        <div className="shell-sidebar__inner">
          ...
          <div className="shell-nav-card">
            ...
            {accountLinks.map((link) => (
              <NavItem key={link.id} link={link} onNavigate={onClose} />
            ))}
            <button className="shell-nav-link" type="button" onClick={onLogout}>
              <Icon name="logout" />
              <span>Salir</span>
            </button>
          </div>
