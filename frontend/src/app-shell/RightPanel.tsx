...
export function RightPanel({ isOpen }: RightPanelProps) {
  const { user } = useAuth()
  const links = getVisibleNavItems(user).filter((item) => item.showInRightPanel)
    ...
    <aside className={`shell-right-panel ${isOpen ? 'active-sidebar' : ''}`} aria-label="Actividad y alertas">
      ...
      {links.length > 0 && (
        <div className="shell-right-card">
          ...
          {links.map((link) => <Link key={link.id} className="shell-mini-link" to={link.to}><Icon name={link.icon} /> {link.label}</Link>)}
        </div>
      )}
    </aside>
  )
