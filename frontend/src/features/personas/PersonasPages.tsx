...
export function BeneficiariosPage() {
  ...
  return <BeneficiariosList key={`${list.params.rama_id ?? ''}-${list.params.unidad_id ?? ''}`} list={list} />
}

function BeneficiariosList({ list }: { list: ReturnType<typeof useListSearchParams> }) {
  const [ramaId, setRamaId] = useState(() => String(list.params.rama_id ?? ''))
  const [unidadId, setUnidadId] = useState(() => String(list.params.unidad_id ?? ''))
  const ramas = useQuery({ queryKey: ['catalogos', 'ramas', { activa: true }], queryFn: async () => { try { return await getRamas({ activa: true }) } catch (error) { throw toApiError(error) } } })
  ...
  const ramaOptions = ramas.data?.data.map((rama) => ({ value: String(rama.id), label: rama.nombre })) ?? []
  const unidadOptions = unidades.data?.data.map((unidad) => ({ value: String(unidad.id), label: `${unidad.nombre} · ${unidad.grupo_nombre}${unidad.estado === 'INACTIVA' ? ' · Inactiva' : ''}` })) ?? []
  const ramaEmpty = ramas.isError ? 'No fue posible cargar ramas.' : ramaOptions.length === 0 && !ramas.isLoading ? 'No hay ramas disponibles.' : 'Todas'
  const unidadEmpty = !ramaId ? 'Selecciona una rama primero' : unidades.isError ? 'No fue posible cargar unidades.' : unidadOptions.length === 0 && !unidades.isLoading ? 'No hay unidades disponibles.' : 'Todas'
  const filters: ListFilter[] = [searchFilter, stateFilter,
    { name: 'rama_id', label: 'Rama', type: 'select', options: ramaOptions, value: ramaId, loading: ramas.isLoading, disabled: ramas.isError, emptyOption: ramaEmpty, onChange: (event) => { setRamaId(event.target.value); setUnidadId('') } },
    { name: 'unidad_id', label: 'Unidad', type: 'select', options: unidadOptions, value: unidadId, loading: unidades.isLoading, disabled: !ramaId || unidades.isError, emptyOption: unidadEmpty, onChange: (event) => setUnidadId(event.target.value) },
  ]
