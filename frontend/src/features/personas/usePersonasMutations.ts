...
export const useUnidadesSeleccionQuery = (ramaId?: number, page = 1) => useQuery({ queryKey: personasQueryKeys.unidades(ramaId, page), enabled: Boolean(ramaId), queryFn: () => request(() => getUnidades({ rama_id: ramaId, estado: 'ACTIVA', page })) })
