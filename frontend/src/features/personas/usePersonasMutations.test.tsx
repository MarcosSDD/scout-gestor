...
vi.mock('../../api/personasApi', () => ({ patchBeneficiarioAsignacion: vi.fn() }))

describe('useAsignacionMutation', () => {
  it('invalida datos de persona, grupos y dashboard tras reasignar', async () => {
    vi.mocked(patchBeneficiarioAsignacion).mockResolvedValue({ success: true, message: 'OK', data: {} } as Awaited<ReturnType<typeof patchBeneficiarioAsignacion>>)
    ...
    const { result } = renderHook(() => useAsignacionMutation(8), { wrapper: Wrapper })
    await act(async () => { await result.current.mutateAsync({ rama_actual: 2, unidad: 4 }) })
