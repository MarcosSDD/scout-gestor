...
export function DashboardPage() {
  ...
  const grupos = useMemo(() => gruposQuery.data?.data ?? [], [gruposQuery.data])
  const selectedGrupoId = useMemo(() => {
    if (grupos.length === 0) {
      return null
    }

    const manualGrupoExists = grupos.some((grupo) => grupo.id === manualGrupoId)
  ...
  if (gruposQuery.isError) {
    return <DashboardErrorState error={gruposQuery.error as unknown as ApiError} onRetry={() => void gruposQuery.refetch()} />
  }
  ...
  if (grupos.length === 0) {
    return (
      <article className="home-card dashboard-state-card">
        ...
        <p>No hay grupos disponibles para tu perfil. Si necesitas acceso, solicita revision de permisos al responsable del sistema.</p>
      </article>
    )
  }

  if (dashboardQuery.isError) {
    return <DashboardErrorState error={dashboardQuery.error as unknown as ApiError} onRetry={() => void dashboardQuery.refetch()} />
  }
  ...
  return (
    <section className="home-feed dashboard-page" aria-label="Dashboard inicial">
      <article className="home-card home-card--hero dashboard-hero-card">
        <div>
          <p className="shell-panel-caption">Dashboard</p>
          <h1>{grupo.nombre_oficial}</h1>
          <p>Resumen inicial del grupo, KPIs operativos y alertas de cumpleanos de los proximos 7 dias.</p>
        </div>
        ...
        <div className="dashboard-hero-actions">
          ...
          {grupos.length > 1 && (
            <label className="dashboard-group-select">
              <span>Grupo</span>
              <select value={selectedGrupoId ?? ''} onChange={(event) => setManualGrupoId(Number(event.target.value))}>
                {grupos.map((item) => <option key={item.id} value={item.id}>{item.nombre_oficial}</option>)}
              </select>
            </label>
          )}
        </div>
      ...
      <div className="dashboard-kpi-grid">
        <KpiCard label="Miembros" value={kpis.total_miembros} detail="Beneficiarios y adultos activos" />
        <KpiCard label="Beneficiarios" value={kpis.total_beneficiarios_activos} detail="Beneficiarios activos del grupo" tone="success" />
        <KpiCard label="Adultos" value={kpis.total_adultos_activos} detail="Adultos activos con rol en unidad" />
        <KpiCard
