...
describe('DashboardPage', () => {
  ...
  it('loads first accessible group automatically and renders KPIs', async () => {
    ...
    expect(screen.getByRole('status')).toHaveTextContent('Cargando grupos accesibles')
  ...
  it('shows selector and loads selected group when multiple groups are available', async () => {
    ...
    vi.mocked(getGrupoDashboard)
      ...
      .mockResolvedValueOnce({
        ...dashboardEnvelope,
        data: { ...dashboardEnvelope.data, grupo: { id: 8, nombre_oficial: 'Grupo Pehuen', estado_vigencia: 'ACTIVO' } },
      })
    ...
    expect(await screen.findByRole('heading', { name: 'Grupo Pehuen' })).toBeInTheDocument()
  })

  it('shows empty state when no accessible groups exist', async () => {
    vi.mocked(getGrupos).mockResolvedValueOnce({ success: true, message: 'OK', data: [] })
  ...
  it('handles backend permission errors from dashboard endpoint', async () => {
    ...
    vi.mocked(getGrupoDashboard).mockRejectedValueOnce({
      ...
      response: {
        data: {
          success: false,
          error: { code: 'permission_denied', message: 'No tiene permisos', details: null },
        },
      },
    })
