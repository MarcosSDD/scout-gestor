...
export function DashboardErrorState({
  ...
  return (
    <article className="home-card dashboard-state-card" role="alert">
      ...
      <p>
        {kind === "unauthorized"
          ? "Tu sesión ya no permite acceder al dashboard. Inicia sesión nuevamente."
          : kind === "forbidden"
            ? "El backend no permite ver el dashboard de este grupo."
            : kind === "not-found"
              ? "El grupo seleccionado no existe o ya no está disponible."
              : (error?.error.message ??
