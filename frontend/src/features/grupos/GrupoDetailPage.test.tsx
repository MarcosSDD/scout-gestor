...
describe("GrupoDetailPage", () => {
  ...
  it("renders detail and the visible hierarchy with RN-05 alerts", async () => {
    vi.mocked(getGrupo).mockResolvedValue({
      success: true,
      message: "OK",
      data: {
        id: 7,
        ...
        direccion: "Av. Scout 123",
        comuna: "Santiago",
        referencia: "",
        latitud: null,
        longitud: null,
        logo: "",
        minimo_miembros_calculado: 20,
        created_at: "2026-01-01",
        updated_at: "2026-01-01",
      },
    });
    vi.mocked(getGrupoEstructura).mockResolvedValue({
      success: true,
      message: "OK",
      data: {
        id: 7,
        nombre_oficial: "Grupo We Lemu",
        zona: { id: 1, nombre: "Zona Centro" },
        distrito: { id: 2, nombre: "Distrito Norte" },
        resumen: {
          total_ramas: 1,
          total_unidades: 1,
          total_subgrupos: 0,
                    ...
                    alertas: [
                      {
                        ...
                        message:
                          "Beneficiario fuera del rango etario de la rama",
                      },
                    ],
                  },
                ],
    ...
    renderWithQueryClient(
      <AuthContext value={auth}>
        <MemoryRouter initialEntries={["/app/grupos/7"]}>
          <Routes>
            <Route path="/app/grupos/:grupoId" element={<GrupoDetailPage />} />
          </Routes>
        </MemoryRouter>
      </AuthContext>,
    );
  ...
  it("rejects invalid ids without issuing requests", async () => {
    renderWithQueryClient(
      <AuthContext value={auth}>
        <MemoryRouter initialEntries={["/app/grupos/invalido"]}>
          <Routes>
            <Route path="/app/grupos/:grupoId" element={<GrupoDetailPage />} />
          </Routes>
        </MemoryRouter>
    ...
    expect(
      screen.getByRole("heading", { name: "Grupo no encontrado" }),
    ).toBeInTheDocument();
