...
describe("GruposPage", () => {
  ...
  it("renders accessible groups and submits filters", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Estado de vigencia"), {
      target: { value: "ACTIVO" },
    });
    fireEvent.click(screen.getByRole("button", { name: "Aplicar filtros" }));

    expect(
      await screen.findByRole("link", { name: "Ver grupo" }),
    ).toHaveAttribute("href", "/app/grupos/7");
    expect(getGrupos).toHaveBeenLastCalledWith({
      search: "Lemu",
      estado_vigencia: "ACTIVO",
    });
  });
  ...
  it("shows an empty accessible state", async () => {
    vi.mocked(getGrupos).mockResolvedValue({
      success: true,
      message: "OK",
      data: [],
    });
