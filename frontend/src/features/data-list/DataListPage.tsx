...
export function DataListPage<T extends { id: number }>(
  ...
  if (isLoading)
    return (
      <GrupoStateCard
        ...
        message="Estamos preparando la informacion disponible para tu perfil."
      />
    );
  if (isError)
    return (
      <GrupoStateCard
        title={`No fue posible cargar ${title.toLowerCase()}`}
  ...
  return (
    <section
      className="home-feed data-list-page"
      aria-labelledby="data-list-title"
    >
      <article className="home-card home-card--hero grupos-hero-card">
        <div>
          <p className="shell-panel-caption">{eyebrow}</p>
          <h1 id="data-list-title">{title}</h1>
          <p>{description}</p>
        </div>
        <div className="data-list-hero-actions">
          <span className="dashboard-status-badge">
            {meta?.count ?? items.length} registros
          </span>
          {heroAction}
        </div>
      </article>
      <form
        className="home-card grupos-filters data-list-filters"
        onSubmit={submit}
          ...
          <label key={filter.name}>
            ...
            {filter.type === "select" ? (
              ...
              >
                <option value="">
                  {filter.loading
                    ? `Cargando ${filter.label.toLowerCase()}…`
                    : (filter.emptyOption ?? "Todos")}
                </option>
              ...
              <input
                ...
                defaultValue={String(params[filter.name] ?? "")}
                placeholder={
                  filter.placeholder ??
                  (filter.type === "number" ? "Identificador" : "Buscar")
                }
              />
            )}
          </label>
      ...
      {items.length === 0 ? (
        <GrupoStateCard
          title={
            hasFilters
              ? "Sin resultados"
              : `Sin ${title.toLowerCase()} accesibles`
          }
          message={
            hasFilters
              ? "Prueba con otros filtros."
              : "No hay informacion disponible para tu perfil."
          }
        />
      ) : (
        <ul className="data-list" aria-live="polite">
          {items.map((item) => (
            <li className="home-card data-list__item" key={item.id}>
              {columns.map((column, index) => (
                <div key={column.label}>
                  ...
                  <strong>
                    {index === 0 && detailPath ? (
                      <Link
                        className="data-list__detail-link"
                        to={detailPath(item)}
                        aria-label={`Ver ficha de ${String(column.render(item))}`}
                      >
                        {column.render(item)}
                      </Link>
                    ) : (
      ...
      {(meta?.previous || meta?.next) && (
        <nav
          className="grupos-pagination"
          aria-label={`Paginacion de ${title.toLowerCase()}`}
        >
          <button
            type="button"
