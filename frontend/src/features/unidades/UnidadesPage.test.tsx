...
describe("UnidadesPage", () => {
  it("renders accessible unit data", async () => {
    vi.mocked(getUnidades).mockResolvedValueOnce({
      success: true,
      message: "OK",
      data: [
        {
    ...
    expect(screen.getByLabelText("Nombre de unidad")).toHaveAttribute(
      "placeholder",
      "Buscar por nombre",
    );
    ...
    fireEvent.change(screen.getByLabelText("Nombre de unidad"), {
      target: { value: "Tropa" },
    });
  ...
  it("does not show the new unit action to unauthorized users", async () => {
      ...
      <AuthContext
        value={{
          ...
          user: {
            ...
            persona_id: 1,
            responsable_grupo_ids: [],
            unidad_roles: [{ unidad_id: 4, rol: "ASISTENTE" }],
            is_apoderado: false,
          },
        }}
      >
    ...
    await screen.findByRole("heading", { name: "Unidades" });
    expect(
      screen.queryByRole("link", { name: "Nueva Unidad" }),
