...
export function UnidadesPage() {
  ...
  return <>{(isAdminUser(user) || isGrupoResponsable(user)) ? <p className="detail-actions"><Link className="secondary-link" to="/app/unidades/nueva">Nueva unidad</Link></p> : null}<DataListPage eyebrow="Organizacion" title="Unidades" description="Unidades visibles para tu perfil y sus datos operativos de lectura." items={query.data?.data ?? []} meta={query.data?.meta} filters={filters} params={list.params} columns={columns} detailPath={(item) => `/app/unidades/${item.id}`} isLoading={query.isLoading} isError={query.isError} error={query.error as never} onRetry={() => void query.refetch()} onApplyFilters={list.applyFilters} onPageChange={list.goToPage} /></>
}
