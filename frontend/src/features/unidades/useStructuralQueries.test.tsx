...
vi.mock("../../api/unidadesApi", async (importOriginal) => ({
  ...(await importOriginal<typeof import("../../api/unidadesApi")>()),
  createUnidad: vi.fn(),
  patchUnidad: vi.fn(),
  reassignSubgrupoMiembro: vi.fn(),
}));

describe("structural mutations", () => {
  it("invalidates all affected read domains after a command", async () => {
    vi.mocked(createUnidad).mockResolvedValue({
      success: true,
      message: "OK",
      data: {} as never,
    });
  ...
  it("removes immutable unit context on patch and reassignment sends only destination", async () => {
    vi.mocked(patchUnidad).mockResolvedValue({
      success: true,
      message: "OK",
      data: {} as never,
    });
    vi.mocked(reassignSubgrupoMiembro).mockResolvedValue({
      success: true,
      message: "OK",
      data: {} as never,
    });
    ...
    expect(patchUnidad).toHaveBeenCalledWith(3, {
      nombre: "Nueva",
      tipo_composicion: "",
      estado: "ACTIVA",
      cupo_maximo: null,
