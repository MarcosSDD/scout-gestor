...
vi.mock('../../api/authApi', () => ({ getMe: vi.fn(), login: vi.fn(), logout: vi.fn(), refreshToken: vi.fn() }))

const authUser = { id: 1, username: 'responsable1', email: 'resp1@scouts.cl', first_name: 'Ana', last_name: 'Rojas', is_staff: false, is_superuser: false, persona_id: null, responsable_grupo_ids: [], unidad_roles: [], is_apoderado: false }

function AuthProbe() {
  const { status, user, logout } = useAuth()
  return <div><p>Status: {status}</p><p>User: {user?.username ?? 'none'}</p><button type="button" onClick={() => void logout()}>Logout</button></div>
}

function CacheProbe() {
  const { status, user } = useAuth()
  return <div><p>Estado: {status}</p><p>Usuario: {user?.username ?? 'ninguno'}</p></div>
}

describe('AuthProvider', () => {
  beforeEach(() => { vi.clearAllMocks(); clearAuthSession() })

  it('starts anonymous when there is no refresh token', async () => {
    renderWithQueryClient(<AuthProvider><AuthProbe /></AuthProvider>)
  ...
  it('restores session with refresh token and /me', async () => {
    ...
    vi.mocked(refreshToken).mockResolvedValueOnce({ success: true, message: 'Token refreshed', data: { access: 'new-access', refresh: 'new-refresh' } })
  ...
  it('clears user, authentication state and query cache when the interceptor expires the session', async () => {
    setAuthTokens({ access: 'access-token', refresh: 'stored-refresh' })
    vi.mocked(refreshToken).mockResolvedValueOnce({ success: true, message: 'Token refreshed', data: { access: 'new-access', refresh: 'new-refresh' } })
    vi.mocked(getMe).mockResolvedValueOnce({ success: true, message: 'Authenticated user', data: authUser })
    const { queryClient } = renderWithQueryClient(<AuthProvider><CacheProbe /></AuthProvider>)
    await screen.findByText('Estado: authenticated')
    queryClient.setQueryData(['private'], 'value')
    notifySessionExpired()
    await waitFor(() => expect(screen.getByText('Estado: anonymous')).toBeInTheDocument())
