...
export function applyDrfErrors<T extends FieldValues>(details: unknown, setError: UseFormSetError<T>, allowedFields: readonly Path<T>[]): string[] {
  if (!details || typeof details !== 'object' || Array.isArray(details)) return messages(details)
  const allowed = new Set<string>(allowedFields)
  const globalErrors: string[] = []
  for (const [field, value] of Object.entries(details as DrfDetails)) {
    const fieldMessages = messages(value)
    if (allowed.has(field) && !(value && typeof value === 'object' && !Array.isArray(value))) {
      setError(field as Path<T>, { type: 'server', message: fieldMessages.join(' ') })
    } else {
